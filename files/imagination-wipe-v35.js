...
class ImaginationWipe {
    ...
    async generateImageForPipeline(entryId, text, speakerName) {
        try {
            ...
            const imageUrl = await this.callGeminiImageAPI(sceneDescription);
    ...
    createControlPanel() {
        ...
        this.panel.innerHTML = `
            <div class="ip-header"><span class="ip-title">💭 想像ワイプ v3.5</span><button class="ip-close">×</button></div>
            <div class="ip-body">
    ...
    async generateImage(text, character = '') {
        ...
        this.updateStatus('シーンを分析中...', 'generating');
        
        try {
            const sceneDescription = await this.analyzeSceneForImage(text, character);
            this.updateStatus('画像を生成中...', 'generating');
            const imageUrl = await this.callGeminiImageAPI(sceneDescription);
            if (imageUrl) { this.displayImage(imageUrl, sceneDescription); this.updateStatus('生成完了！', 'success'); }
...
console.log('🎨 imagination-wipe-v3.5.js ロード完了（監視対象選択 + Grok+BBS送信対応）');
